...
class DFFExample2() extends Component {
  val io = new Bundle{
  ...
  //val dataIn = in UInt (4 bits)
  //val dataOut = out UInt (4 bits)
  var reg000 = RegFactory(UInt(4 bits), config000)
  ...
  reg000.allowOverride
  when (io.set1 ){reg000 :=1}//set result to 1
  //when (io.set1 ){reg000.setAll() }//set all bits to 1
...
object try2 {
  def main(args: Array[String]) {
    SpinalSystemVerilog(new DFFExample2())
  }
}